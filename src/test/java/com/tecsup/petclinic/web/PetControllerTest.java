...
public class PetControllerTest {
     ...
     */
    @Test
    public void testUdatePet() throws Exception {
    	
    //CREO UN NUEVO PET

    	String NAME_PET = "Beethoven4";
		int TYPE_ID = 1;
		int OWNER_ID = 1;
		Date DATE = new SimpleDateFormat("yyyy-MM-dd").parse("2020-05-20");
		...
		String UP_NAME_PET = "Beethoven5";
		int UP_TYPE_ID = 1;
		int UP_OWNER_ID = 1;
		Date UP_DATE = new SimpleDateFormat("yyyy-MM-dd").parse("2020-05-20");
		
		Pet UP_Pet = new Pet(UP_NAME_PET, UP_TYPE_ID, UP_OWNER_ID ,UP_DATE);
		
		String response = mvcActions.andReturn().getResponse().getContentAsString();
		Integer id = JsonPath.parse(response).read("$.id");
		
		ResultActions mvcActionsUP = mockMvc.perform(put("/pets/"+ id)
	            .content(om.writeValueAsString(UP_Pet))
	            .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON))
	            .andDo(print())
	            .andExpect(status().isOk());     
